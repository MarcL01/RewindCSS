...
const parseValue = (
	class_: ClassName,
	classType: ClassType,
	specialClassValues: ClassName[],
	valueType: ClassValueType,
) => {
	if (
		classType === (class_ as unknown) ||
		(valueType === "special" && specialClassValues.some((s) => s === class_))
	) {
		return class_;
		...
		if (classValueString.match("%[").size() > 0) {
			...
			if (isUDim || isRounded) {
				...
				if (classArbitraryValue.match("^rgb").size() > 0) {
					const [r, g, b] = classArbitraryValue
						.sub(5, -2)
						.split(",")
						.map((c) => tonumber(c)!)
			...
			const hasUDim = isUDim && Object.keys(values.udim).some((u) => u === classValueString);

			const isColor3 = valueType === "color3" && Object.keys(values.color3).some((u) => u === classValueString);

			const isVector2 =
				valueType === "vector2" && Object.keys(values.vector2).some((u) => u === classValueString);

			const isZ = valueType === "z" && Object.keys(values.z).some((u) => u === classValueString);

			const isBorder = valueType === "border" && Object.keys(values.border).some((u) => u === classValueString);

			const isRounded =
				valueType === "rounded" && Object.keys(values.rounded).some((u) => u === classValueString);

			const isOpacity =
				valueType === "opacity" && Object.keys(values.opacity).some((u) => u === classValueString);

			const isLeading =
				valueType === "leading" && Object.keys(values.leading).some((u) => u === classValueString);

			const isText = valueType === "text" && Object.keys(values.text).some((u) => u === classValueString);

			const isFontWeight =
				valueType === "font-weight" && Object.keys(values.fontWeight).some((u) => u === classValueString);

			const isAspect = valueType === "aspect" && Object.keys(values.aspect).some((u) => u === classValueString);

			const isScale = valueType === "scale" && Object.keys(values.scale).some((u) => u === classValueString);

			const isMinH = valueType === "min-h" && Object.keys(values.minH).some((u) => u === classValueString);

			const isMinW = valueType === "min-w" && Object.keys(values.minW).some((u) => u === classValueString);

			const isMaxH = valueType === "max-h" && Object.keys(values.maxH).some((u) => u === classValueString);

			const isMaxW = valueType === "max-w" && Object.keys(values.maxW).some((u) => u === classValueString);
